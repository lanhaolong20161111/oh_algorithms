...
public:
    ...
    int Count(vector<vector<char>>& board,int x,int y) { //求(x,y)相邻的'M'数
        int nx,ny,cnt=0;
    ...
    void bfs(vector<vector<char>>& board,int x,int y) {     //广度优先搜索
        int nx,ny,cnt;
        ...
        while(!qu.empty()) {
            ...
            for(int di=0;di<8;di++){
                ...
                if(board[nx][ny]=='B')          //为'B'时跳过
                    continue;
                cnt=Count(board,nx,ny);         //求(nx,ny)周围的地雷数
                if(cnt==0) {                       //cnt=0:规则2
                    board[nx][ny]='B';
                    qu.push(pair<int,int>(nx,ny));
                }
                else                        //cnt>0:规则3
                    board[nx][ny]=cnt+'0';    //转换为数字字符
            }
        }
    }
};
